...
## 1.7.0-luoluo.2 — 二改开发快照 (2026-09-21)

- 对应原版仍为 `v1.7.0` / `b01a0c506f4f10f90f30d408c0291f16720f0718`，未升级上游基线。
- 新增角色范围 `float-memory-v1` API：事件幂等写入、全局/群聊召回、删除后收据保留；配套 float `v0.2.0`。
- 保护群事件原文免于 Dream 转换与软化，自动 Dream 活动判断纳入服务端 float 收据时间。
- 保留 `v1.7.0-luoluo.1` 标签；版本定位、已知限制与回退步骤见 [版本记录](docs/versions/v1.7.0-luoluo.2.md)。本标签用于源码追踪，不代表生产或完整发布验收通过。
